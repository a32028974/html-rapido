...
function getRetiroTipo() {
  ...
  if (!sel) return "";
  switch (sel.value) { case "7": return "NORMAL"; case "3": return "URGENTE"; case "15": return "LABORATORIO"; default: return ""; }
}
function showMsg(text, color="green"){ const p=$("mensaje"); if(!p) return; p.textContent=text; p.style.color=color; }
    ...
    if (res.ok && text.startsWith("✅")) {
      ...
      if (window.Swal) {
        Swal.fire({
          ...
          if (r.isConfirmed) {
            if (window.__buildPrintArea) window.__buildPrintArea();
            window.print();
          }
        });
      }
